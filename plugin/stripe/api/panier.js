// AJOUTER AU PANIER
var articles = document.querySelectorAll('.liste-produits--details');
...
articles.forEach(function(article) {
	...
	article.querySelector('button').addEventListener('click', function(){
        ...
        formData.append('title', article.querySelector("[data-item='title']").innerText);
        formData.append('pu',article.querySelector("[data-item='pu']").innerText);

        xhr.open('POST', 'theme/'+theme+'/plugin/stripe/api/panier.ajax.php?mode=ajouter&id='+id);
        ...
        xhr.onload = function()
        {
            ...
            document.querySelector(".panier--quantite").setAttribute("data-quantite",parseInt(data['qte']));
            light('Product added to cart', 1000);
...
articles.forEach(function(article) {
	...
	article.addEventListener('change', function(){
        ...
        xhr.open('GET', path+'theme/'+theme+'plugin/stripe/api/panier.ajax.php?mode=modifier&id='+id+'&qte='+this.value,true);
...
articles.forEach(function(article) {
	...
	article.addEventListener('click', function(){
        ...
        xhr.open('GET', path+'theme/'+theme+'plugin/stripe/api/panier.ajax.php?mode=supprimer&id='+id,true);
...
document.querySelector('#payer').addEventListener('click', function(event){
    ...
    var totalCommande = document.querySelector("[data-item='total-panier']").innerText;

...
switch(@array_keys($GLOBALS['filter'])[0]) {
   ...
   <form id="<?_e('cart')?>" method="POST" class="panier">

      <h2>
         <?_e('Cart')?>
         <aside class="inbl panier--quantite" data-quantite="<?=count($_SESSION['panier'])?>"></aside>
      </h2>
      ...
      // on affiche le détail du panier
      else {
         $enabled = 'enable';
         ...
         if(@array_keys($GLOBALS['filter'])[0] == 'annulation' && @$_SESSION['checkout_session']['id']) 
         {
            ...
            require_once('/api/stripe/init.php');
         ...
         <div id="<?=make_url(_e('Delivery charges'))?>" class="">
      
            <h3><?_e('Delivery charges')?></h3>
...
<?
      ...
      <p>
         ...
         <span data-item="total-panier" data-devise="€"><?=($total_panier > 0 ? $total_panier : '0,00')?></span>

      </p>

      <div>
         <button id="payer" class="bt" <?=$enabled?>><?php _e('Checkout')?></button>
      </div>
